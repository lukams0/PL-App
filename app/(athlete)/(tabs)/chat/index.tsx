import ChatConversationComponent from '@/components/athlete/chat/ChatConversationComponent';
import ChatListComponent from '@/components/athlete/chat/ChatListComponent';
...
// Match shapes used by your components
type ChatPreview = {
...
export default function ChatPage() {
    ...
    const currentUserId = user?.id ?? ''; // adjust if your auth shape differs
    ...
    const [currentUserChatPeer, setCurrentUserChatPeer] = useState<ChatUser | null>(null);
    const { refreshUnread } = useChatBadge();
    // -------------- Presence --------------
    useEffect(() => {
    ...
    const filteredChats: ChatPreview[] = useMemo(() => {
        ...
        const withPresence = chats.map((chat) => ({
            ...chat,
            isOnline: onlineIds.includes(chat.userId),
        }));
    ...
    // -------------- Open conversation --------------
    useEffect(() => {
        // If we were navigated with a conversationId param, open it once chats are loaded
    ...
    const handleChatPress = async (chatId: string) => {
        ...
        if (selectedChat) {
            const isOnline = onlineIds.includes(selectedChat.userId);
            setCurrentUserChatPeer({
        ...
        try {
            ...
            // 🔹 Optimistically zero unread *in the local list state*
            setChats((prev) =>
                prev.map((c) =>
    ...
    const handleSendMessage = async () => {
        if (!messageText.trim() || !selectedChatId || !currentUserChatPeer || !currentUserId) return;
    ...
    const handleNewChat = () => {
        router.push('/chat/new'); // adjust path to your route layout if needed
    };
    ...
    const handleBack = () => {
        ...
        setMessages([]);
        setMessageText('');
        setCurrentUserChatPeer(null);
    };

    const handleUserProfile = () => {
        if (currentUserChatPeer) {
            console.log('Navigate to user profile:', currentUserChatPeer.id);
            // router.push(`/profile/${currentUserChatPeer.id}`);
        }
    };
    ...
    if (selectedChatId && currentUserChatPeer) {
        return (
            <ChatConversationComponent
                user={currentUserChatPeer}
                messages={messages}
